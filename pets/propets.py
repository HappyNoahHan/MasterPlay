...
class Fire(pets.pet.Pet):
    def __init__(self, level=1,skill_list={},exp_for_current=0,realize_skill_list=[],status=[],carry_prop=None,indi_list=rancom.getIndiValue()):
        super().__init__(level=level,skill_list=skill_list,exp_for_current=exp_for_current,realize_skill_list=realize_skill_list,status=status,carry_prop=carry_prop,indi_list=indi_list)
